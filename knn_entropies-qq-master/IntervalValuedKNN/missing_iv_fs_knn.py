...
if __name__ == '__main__':
    ...
    aggregations = (AMean(),)
    prec_aggregations = (A1(), A2(), A3(), A4(), A3('p'), A4('p'), AMean(),)
    ...
    for k in k_neighbours:
        for aggg in aggregations:
            for s_agg in similarity_aggregations:
                for p_agg in prec_aggregations:
                    for ord in orders:
                        for meth in meths:
                            for prec in precedences:
                                ...
                                for i in range(attempts):
                                    if entropy and prec == 'p':
                                        ord = "possible"
                                    if entropy and prec == 'n':
                                        ord = "neccesary"

                                    # if entropy and isinstance(s_agg, AMean):
                                    #     prec = 'n'
                                    ...
                                    X_test = np.load(
                                        path_prefix + data_slice + 'X_test' + str(miss_rate) + str(i) + '.npy')
                                    y_train = np.load(
                                        path_prefix + data_slice + 'y_train' + str(miss_rate) + str(i) + '.npy')
                                    y_test = np.load(
                                        path_prefix + data_slice + 'y_test' + str(miss_rate) + str(i) + '.npy')

                                    X_train = fill_missing_values(X_train, X_train, type=type, k_neighbours=k,
                                                                  meth=meth)
                                    ...
                                    knn = IntervalValuedFuzzyKNN(similarity_aggregation=s_agg, k_neighbours=k,
                                                                 aggregation=aggg,
                                                                 order=ord, precedence=prec, prec_aggregation=p_agg,
                                                                 weighted_similarity=False)
                                ...
                                average = pd.DataFrame(
                                    {'accuracy': np.mean(accuracy), 'sensitivity': np.mean(sensitivity),
                                     ...
                                     'similarity aggregation': s_agg.aggregation_name(), 'order': ord,
                                     'precedence': prec,
